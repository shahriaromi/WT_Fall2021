   ...
   <label for="language2"> PHP</label>
    ...
    <?php
      ...
      if (isset($_POST['submit'])){
        ...
        $datas = array("firstName"=> $firstName, "lastName"=> $lastName, "email"=> $email, "password"=> $password, "age"=> $age, "designation"=> $designation, "language"=> "$check1  $check2  $check3" );


        $jsonDatas = json_encode($datas);
        
        file_put_contents('mydata.json', $jsonDatas);
